...
def _try_llm_synthesis(question: str, passages: list[Passage]) -> str | None:
    ...
    try:
        ...
        if backend == "ollama":
            ...
            host = os.environ.get("EDGEAI_OLLAMA_HOST", "http://localhost:11434").rstrip("/")
            # Cap the context window: RAG prompts are small (a few passages), so
            # a large default context (e.g. 32K) just wastes KV-cache VRAM and can
            # force layers onto the CPU. 8192 keeps the model fully on an 8GB GPU.
            num_ctx = int(os.environ.get("EDGEAI_OLLAMA_NUM_CTX", "8192"))
            r = requests.post(
                f"{host}/api/generate",
